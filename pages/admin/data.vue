...
function displayVal(col: ColMeta, val: any): string {
  ...
  if (col.uiType === 'json') return typeof val === 'object' ? JSON.stringify(val) : String(val)
  return String(val)
}
...
<template>
  <div class="mx-auto max-w-7xl px-4 py-6" v-if="can('data:view')">
    ...
    <div class="flex gap-4">
      <!-- 左：分类-表树 -->
      <aside class="w-56 shrink-0 rounded-xl bg-gray-50 p-3">
        <div v-for="cat in categories" :key="cat.key" class="mb-3">
          <p class="mb-1 px-1 text-xs font-semibold text-gray-500">{{ cat.label }}</p>
          <button
            v-for="t in (tablesByCat[cat.key] || [])"
            :key="t.key"
            class="mb-0.5 block w-full rounded-lg px-2 py-1.5 text-left text-sm"
            :class="activeTable === t.key ? 'bg-primary text-white' : 'text-gray-700 hover:bg-gray-200'"
      ...
      <section class="min-w-0 flex-1">
        ...
        <template v-else>
          <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <button v-if="can('data:create')" class="rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700" @click="startAdd">+ 新增行</button>
            ...
            <table class="w-full text-left text-sm">
              <thead class="bg-gray-50 text-xs text-gray-500">
                <tr>
                  ...
                  <th class="px-3 py-2 font-medium" v-if="can('data:edit') || can('data:delete')">操作</th>
                </tr>
              </thead>
              <tbody>
                <!-- 新增行 -->
                <tr v-if="adding" class="bg-blue-50/40">
                  <td v-for="c in columns" :key="c.name" class="px-2 py-1.5 align-top">
                    ...
                    <textarea v-else-if="c.uiType === 'json'" v-model="addCopy[c.name]" rows="2" class="w-full rounded border border-gray-200 px-1 py-1 text-xs font-mono"></textarea>
                ...
                <!-- 数据行 -->
                <tr v-for="row in rows" :key="row[primaryKey]" class="border-t border-gray-100 hover:bg-gray-50">
                  <!-- 编辑态 -->
                  <template v-if="editingId === row[primaryKey]">
                    <td v-for="c in columns" :key="c.name" class="px-2 py-1.5 align-top">
                      ...
                      <textarea v-else-if="c.uiType === 'json'" v-model="editCopy[c.name]" rows="2" class="w-full rounded border border-gray-200 px-1 py-1 text-xs font-mono"></textarea>
                  ...
                  <template v-else>
                    <td v-for="c in columns" :key="c.name" class="px-3 py-2 align-top">
                      <span v-if="c.isFk">{{ fkLabel(c.name, row[c.name]) }}</span>
                      <span v-else-if="c.uiType === 'json'" class="font-mono text-xs text-gray-500 break-all">{{ displayVal(c, row[c.name]) }}</span>
                      <span v-else>{{ displayVal(c, row[c.name]) }}</span>
                    </td>
    ...
    <div v-if="showImport" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="showImport = false">
      <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p class="mb-3 text-xs text-gray-400">覆盖模式会先清空本表（含其从表）再导入；导入前请确认已备份。</p>
