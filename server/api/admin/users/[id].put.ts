...
export default defineEventHandler(async (event) => {
  ...
  if (body.password) {
    ...
    await db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(ph, id)
  }
  // 基本信息
  if (body.username !== undefined) {
    const uname = String(body.username).trim()
    if (uname.length < 2) throw createError({ statusCode: 400, statusMessage: '用户名至少 2 个字符' })
    if (await db.prepare('SELECT id FROM users WHERE username = ? AND id <> ?').get(uname, id)) {
  ...
  if (body.email !== undefined) {
    ...
    await db.prepare('UPDATE users SET email = ? WHERE id = ?').run(em, id)
  }
  if (body.phone !== undefined) {
    const ph = body.phone ? String(body.phone).trim() : null
    await db.prepare('UPDATE users SET phone = ? WHERE id = ?').run(ph, id)
  }

  // 角色（重新分配）
  if (Array.isArray(body.roleCodes)) {
    const placeholders = body.roleCodes.map(() => '?').join(',')
    const roleIds = await db.prepare(`SELECT id FROM roles WHERE code IN (${placeholders})`).all(...body.roleCodes) as { id: number }[]
    const tx = db.transaction(async () => {
      ...
      for (const r of roleIds) await ins.run(id, r.id)
    })
    await tx()
    const hasAdmin = body.roleCodes.includes('admin')
    await db.prepare('UPDATE users SET role = ? WHERE id = ?').run(hasAdmin ? 'admin' : 'user', id)
  }
