...
import { PARAM_KEY_LABELS } from './paramKeys'
...
【为什么不把它们接进来（2026-09-18 复核后决定，不再是待办）】
曾计划「补开发 / 运维类别列 → 把「行业基准参数」的取值搬进本表 → 引擎改读本表」。复核两边数据后放弃了：
· 本表只有 4 条左右（更老的宽表遗留），而「行业基准参数」覆盖 14 套标准 / 99 行 —— 这不叫接线，叫搬整个家；
· 本表是列式：新增一套标准若带来表里没有的参数种类（如某省的「安全等级调整因子」）就得改表结构 =
  发版迁移；「行业基准参数」是行式，加标准只插行、不动结构 —— 行式反而更扛得住「不断加标准」；
· 本表要装「多选项」取值（生产率 5 个分位、应用类型 8 项）只能塞回 JSON 列，等于退回当初做 3NF 重构要拆掉的宽表形态。
结论：真正的病根不是「取值放在哪张表」，而是「引擎按中文名匹配取值」—— 已由「行业基准参数」
新增的「参数键」列根治。本表继续管它已经在管的两件事：本标准的算法、本标准的复杂度判定矩阵。`,
  },
...
【参数键是最要紧的一列】引擎只按「参数键」认参数，「参数名称」现在只是给人看的标签。
· 改「参数名称」是安全的 —— 那只是文字，改了不影响任何金额。以前不是这样：引擎按中文名取数，
  改个名就静默掉到兜底值，而兜底 HM(174) 恰好等于多数标准的取值，失配被数值巧合掩盖
  （只有北京的 176 会悄悄变 174，金额小改、不报错、不留痕）—— 参数键就是为根治这个而加的。
· 「参数键」留空 = 本行不进引擎，仅作页面展示。本表里留空的行大多是这类参考项
  （如「运维对象特征-用户规模」「各工程活动工作量分布」「系统更新频率」）。
· 新增一条要被引擎读取的取值（比如某省标准的人月费率），必须选对参数键，否则测算里根本看不到它。
· 可选的键见本列下拉：人月折算系数 / 人月费率 / 开发基准生产率 / 运维基准生产率 /
  应用类型调整因子 / 开发平台调整因子 / 开发团队背景调整因子 / 非功能性特征调整因子 /
  规模变更因子 / 复用度调整因子。每个键的中文含义与「引擎拿它干什么」见 server/config/paramKeys.ts。
【和另外几张标准表的分工（最容易搞混的地方）】
· 造价标准 = 标准目录 + 编号桥接（引擎靠它的「代号」把两套编号对上），不存取值；
· 标准参数明细 = 只给 /standards 页展示看，不参与计算；
· 标准基准取值 = 只声明「这套标准用哪套算法、哪套复杂度矩阵」，其余取值列暂未接入；
...
【参数类型】weight 权重 / factor 系数 / rate 费率 / productivity 生产率 / formula 公式。
「生产率」这一类型有额外作用：引擎靠它判断「这套标准给出过生产率」——
像山东的「按业务领域 P50 参考CSBMK」取值指向外部基准、取不出数字，也照样按兜底生产率处理，
而不是当成「这套标准没给生产率」。
...
export const DATA_LABELS: Record<string, string> = {
  ...
  'estimation_parameters.param_key': '参数键',
...
export const DATA_ENUMS: Record<string, Record<string, string>> = {
  ...
  // 参数键的中文名与说明统一取自 server/config/paramKeys.ts（唯一事实源），不在这里重抄一份
  'estimation_parameters.param_key': PARAM_KEY_LABELS,
